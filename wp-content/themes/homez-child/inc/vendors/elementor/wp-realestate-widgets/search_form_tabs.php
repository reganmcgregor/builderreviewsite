	...
	protected function register_controls() {
        ...
        $repeater->add_control(
            ...
            [
                ...
                'default' => 'select',
                'condition' => [
                    'filter_field' => ['type', 'type', 'location', 'label', 'amenity', 'material'],
                ],
            ]
        );
                ...
                <?php if ( !empty($tabs_search) ) { ?>
                    <ul class="tab-list nav-tabs nav">
                        <?php foreach ($tabs_search as $tab) { ?>
                            <li>
                                <label>
                                    <input type="radio" name="filter-type" value="<?php echo esc_attr(isset($tab['tab_type']) ? $tab['tab_type'] : ''); ?>" <?php checked($active_tab, $tab['tab_type'], true); ?>>
                                    <span>
                                        <?php
    ...
    public function form_fields_display($search_fields, $filter_fields, $instance, $args) {
        ...
        if ( !empty($search_fields) ) {
            foreach ($search_fields as $item) {
                ...
                if ( $item['filter_layout'] && in_array($item['filter_field'], array('type', 'type', 'location', 'label', 'amenity', 'material')) ) {
