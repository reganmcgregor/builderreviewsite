<?php
/**
 * Custom template tags for Homez
 *
...
if ( ! function_exists( 'homez_comment_nav' ) ) {
	function homez_comment_nav() {
		// Are there comments to navigate through?
		if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
		?>
		<nav class="navigation comment-navigation" role="navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'homez' ); ?></h2>
			<div class="nav-links">
				<?php
					if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'homez' ) ) ) :
						printf( '<div class="nav-previous"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> %s</div>', $prev_link );
					endif;

					if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'homez' ) ) ) :
...
if ( ! function_exists( 'homez_post_categories' ) ) {
	function homez_post_categories( $post ) {
...
if ( ! function_exists( 'homez_post_categories_first' ) ) {
	function homez_post_categories_first( $post ) {
		$cat = wp_get_post_categories( $post->ID );
		echo '<div class="list-categories">';
		foreach ( $cat as $c ) {
			$categories = get_category( $c );
...
if ( ! function_exists( 'homez_post_thumbnail' ) ) {
	function homez_post_thumbnail($thumbsize = '', $link = '') {
		...
		if ( is_singular('post') && is_single($post) ) {
			...
			$html .= homez_get_attachment_thumbnail($post_thumbnail_id, 'full');
			$html .= '</div>';
			...
			$html .= '<figure class="entry-thumb">';
				$html .= '<a class="post-thumbnail" href="'.esc_url($link).'" aria-hidden="true">';
						$post_thumbnail_id = get_post_thumbnail_id();
						$html .= homez_get_attachment_thumbnail($post_thumbnail_id, $thumbsize);
				$html .= '</a>';
...
if ( ! function_exists( 'homez_display_post_thumb' ) ) {
	function homez_display_post_thumb($thumbsize) {
		$post_format = get_post_format();
		$output = '';
		
        if ( has_post_thumbnail() ) {
            $output = homez_post_thumbnail($thumbsize);
        }
	    return $output;
	}
}

function homez_get_attachment_thumbnail($attachment_id, $size = 'thumbnail', $icon = false, $attr = '', $wrapper = true) {
	$html = '';
	...
	if ( $image ) {
		...
		if ( homez_get_config('image_lazy_loading') ) {
			$src_layzy = homez_create_placeholder(array($width, $height));
			$attr['data-src'] = $src;
			$attr['src'] = $src_layzy;
