...
function homez_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'homez' ),
        'preview_url'                  => 'https://demoapus1.com/homez/',
    );
    return apply_filters( 'homez_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'homez_ocdi_import_files' );

function homez_ocdi_after_import_setup( $selected_import ) {
    ...
    if ( file_exists($file) ) {
        homez_ocdi_import_settings($file);
    }
...
add_action( 'pt-ocdi/after_import', 'homez_ocdi_after_import_setup' );

function homez_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        homez_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        homez_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function homez_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function homez_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function homez_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'homez_ocdi_before_widgets_import' );