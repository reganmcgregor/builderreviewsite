...
class Homez_Elementor_Transactions extends Elementor\Widget_Base {
	...
	public function get_title() {
        return esc_html__( 'Apus Transactions', 'homez' );
    }
	...
	protected function render() {
        ...
        extract( $settings );
        ?>
        <h1 class="title-profile"><?php esc_html_e( 'Transactions', 'homez' ) ; ?></h1>
        <div class="box-white-dashboard">
            <div class="inner-list">
                ...
                <?php if ( ! is_user_logged_in() ) {
                    ?>
                        <div class="text-warning"><?php  esc_html_e( 'Please login to see this page.', 'homez' ); ?></div>
                    <?php
                } else {
                        ...
                        <div class="widget-user-packages <?php echo esc_attr($el_class); ?>">
                            <div class="widget-content table-responsive">
                                <table class="user-transactions">
                                    <thead>
                                        <tr>
                                            <td><?php esc_html_e('Order ID', 'homez'); ?></td>
                                            <td><?php esc_html_e('Package', 'homez'); ?></td>
                                            <td><?php esc_html_e('Amount', 'homez'); ?></td>
                                            <td><?php esc_html_e('Date', 'homez'); ?></td>
                                            <td><?php esc_html_e('Payment Mode', 'homez'); ?></td>
                                    ...
                                    <tbody>
                                        <?php while ($trans_loop->have_posts()) : $trans_loop->the_post();
                                            ...
                                            if ($trans_status == 'completed') {
                                                $status_txt = esc_html__('Successfull', 'homez');
                                                $status_class = 'success';
                                            } else if ($trans_status == 'processing') {
                                                $status_txt = esc_html__('Processing', 'homez');
                                                $status_class = 'pending';
                                            } else if ($trans_status == 'refunded') {
                                                $status_txt = esc_html__('Refunded', 'homez');
                                                $status_class = 'pending';
                                            } else {
                                                $status_txt = esc_html__('Pending', 'homez');
                                                $status_class = 'pending';
                                            }
                                            ...
                                            if ($payment_mode == 'cod') {
                                                $payment_mode = esc_html__('Cash on Delivery', 'homez');
                                            }
                                        ?>
                                            <tr>
                    ...
                    <?php } else { ?>
                        <div class="not-found"><?php esc_html_e('Don\'t have any items', 'homez'); ?></div>
                    <?php } ?>
                <?php } ?>
            </div>
        </div>
...
Elementor\Plugin::instance()->widgets_manager->register( new Homez_Elementor_Transactions );