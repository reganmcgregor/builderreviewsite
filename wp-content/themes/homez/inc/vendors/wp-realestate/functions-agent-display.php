...
function homez_agent_display_image($post, $size = 'thumbnail') {
	?>
...
function homez_agent_display_full_location($post, $display_type = 'normal', $echo = true) {
	ob_start();
	...
	if( $display_type == 'icon' ){
		...
		$prefix = '<span class="location-label">'.esc_html__('Location', 'homez').'</span>';
	} else {
		$prefix = '';
	}
	if ( $location ) {
...
function homez_agent_display_nb_properties($post) {
...
function homez_agent_display_nb_views($post) {
	$agent_id = $post->ID;
	...
	?>
	<div class="nb_views">
        <?php echo sprintf(_n('<span class="text-blue">%d</span> <span class="text">View</span>', '<span class="text-blue">%d</span> <span class="text">Views</span>', intval($views), 'homez'), $views_display); ?>
    </div>
    <?php
}

function homez_agent_display_featured_icon($post) {
	$featured = WP_RealEstate_Agent::get_post_meta( $post->ID, 'featured', true );
	if ( $featured ) { ?>
        <span class="featured-icon" data-toggle="tooltip" title="<?php esc_attr_e('featured', 'homez'); ?>"><i class="fas fa-star"></i></span>
    <?php }
}

function homez_agent_display_phone($post, $display_type = 'no-title', $echo = true, $always_show_phone = false) {
...
function homez_agent_display_email($post, $display_type = 'no-title', $echo = true) {
...
function homez_agent_display_website($post, $display_type = 'no-title', $echo = true) {
...
function homez_agent_display_member_since($post, $display_type = 'no-title', $echo = true) {
...
function homez_agent_display_job($post, $echo = true) {
...
function homez_agent_display_meta_data($post, $meta_key, $title = '', $icon = '', $echo = true) {
...
function homez_agent_display_socials($post, $display_type = 'no-title', $echo = true) {
...
function homez_agent_display_rating($post) {
