...
if ( has_post_thumbnail() || ($gallery && is_array($gallery)) ) {
    $gallery_size = !empty($gallery_size) ? $gallery_size : '875x580';
    $gallery_second_size = !empty($gallery_second_size) ? $gallery_second_size : '570x570';

    $first_class = 'col-12';
    $second_class = 'col-sm-12 col-6';
    if ( $gallery && is_array($gallery) ) {
        $first_class = 'col-sm-9 c1 col-12';
    } else {
        $gallery_size = '1170x580';
    }
?>
                ...
                <div class="row row-10">
                    <?php $i=1; foreach ( $gallery as $id => $src ) {
                        ...
                        if ( $i == 2 && count($gallery) > 2 ) {
                            $more_image_html = '<div class="view-more-gallery">'.sprintf(esc_html__('See All %s Photos', 'homez'), count($gallery) - 2).'</div>';
